...
-- Trigger: Créer une notification lors de validation de certificat
CREATE TRIGGER notif_certificat_valide
AFTER UPDATE OF statut ON demandes_certificats
FOR EACH ROW
WHEN NEW.statut = 'VALIDE' AND OLD.statut != 'VALIDE'
BEGIN
    INSERT INTO notifications (user_id, titre, message, type)
    SELECT 
        e.user_id,
        'Certificat disponible',
        'Votre ' || NEW.type_certificat || ' est prêt à être téléchargé',
        'SUCCES'
    FROM etudiants e
    WHERE e.id = NEW.etudiant_id;
END;
